...
describe('AuthDialog', () => {
  it('maps a sign-in failure to a helpful message without reaching Firebase', async () => {
    ...
    expect(authMocks.signIn).toHaveBeenCalledWith({ name: 'test-auth' }, 'learner@example.com', 'not-a-real-password');
  });
  ...
  it('closes after a successful account creation', async () => {
    ...
    expect(authMocks.create).toHaveBeenCalledWith({ name: 'test-auth' }, 'learner@example.com', 'long-enough-password');
