...
const progress = { version: 2, configuration, currentStep: 1, unlockedThroughStep: 1, updatedAt: '2026-01-01T00:00:00.000Z', completedAt: null } as Progress;
...
describe('lab API client', () => {
  it('returns the lab definition from the service envelope', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(expect.stringMatching(/\/labs\/serverless-web$/), expect.objectContaining({
  ...
  it('maps network and server errors to actionable ApiError values', async () => {
    ...
    fetchMock.mockResolvedValueOnce(new Response(JSON.stringify({ code: 'DATABASE_UNAVAILABLE', message: 'Try later.' }), { status: 503 }));
