...
const steps: LabStep[] = [
    {
        ...
        services: ['Amazon S3'],
        goal: 'Keep the site private.',
        concept: 'Use controlled delivery.',
        instructions: ['Configure storage.'],
    },
    {
        id: 'api',
        number: '02',
        ...
        concept: 'Connect HTTP to compute.',
        instructions: ['Configure an API route.'],
    },
    {
        id: 'data',
        number: '03',
        title: 'Protect application data',
        services: ['DynamoDB'],
        goal: 'Store application items.',
        concept: 'Use scoped permissions.',
        instructions: ['Configure data access.'],
    },
    {
        id: 'observe',
        number: '04',
        title: 'Observe the function',
        services: ['CloudWatch'],
        goal: 'Record and notice failures.',
        concept: 'Use logs and alarms.',
...
const lab: Lab = {
    ...
    description: 'Build a small serverless app.',
    duration: '45 minutes',
    level: 'Beginner',
    initialConfiguration: { resources: [] },
    serviceCatalog: steps.map((step, stepIndex) => ({
        category: step.title,
        services: [
            {
                id: ['s3', 'api-gateway', 'dynamodb', 'cloudwatch'][stepIndex],
                name: step.services[0],
                stepIndex,
            },
        ],
    })),
        ...
        steps: steps.map((step, index) => {
            ...
            return {
                ...
                checks: [
                    {
                        id: `check-${index}`,
                        label: `Stage ${index + 1} is configured`,
                        passed: checkPassed,
                        hint: 'Review this stage.',
                    },
                ],
            };
        }),
...
describe('guest learning flow', () => {
    it('shows failed checks, unlocks each stage after validation, and reaches completion', async () => {
        ...
        await waitFor(() =>
            expect(screen.getByRole('heading', { level: 1 })).toHaveTextContent('Learn how cloud'),
        );
        ...
        expect(
            await screen.findByRole('heading', { name: 'You built a serverless web app.' }),
        ).toBeInTheDocument();
