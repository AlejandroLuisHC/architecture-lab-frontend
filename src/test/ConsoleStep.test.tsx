...
const step: LabStep = {
  id: 'delivery', number: '01', title: 'Build a private origin',
  services: ['Amazon S3'], goal: 'Keep the website files private.',
  concept: 'CloudFront can serve files from a private bucket.', instructions: ['Create a bucket.'],
};

function SandboxHarness() {
  const [configuration, setConfiguration] = useState<LabConfiguration>({ resources: [] });
  return <ConsoleStep serviceId="s3" serviceName="Amazon S3" step={step} configuration={configuration} onChange={setConfiguration} />;
}

describe('ConsoleStep resource editing', () => {
  beforeEach(() => {
    Object.defineProperty(window.crypto, 'randomUUID', { configurable: true, value: vi.fn(() => 'bucket-test-id') });
  });
